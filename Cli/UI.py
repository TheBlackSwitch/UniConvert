...
def getfilenames():
    ...
    print("""
+---------------------------------------------------------------------+
| 1) Neem alle bestanden in de huidige map                            |
+---------------------------------------------------------------------+
| 2) Ik wil ze zelf geven                                             |
+---------------------------------------------------------------------+
    """)
    # Vragen welke selectie methoden ze willen gebruiken.
    while True:
        val = input("Typ: stop om af te sluiten | Nummer:").strip()
        if val.isnumeric():
            keuze_bestandsmethode = int(val)
            if 3 > keuze_bestandsmethode > 0:
                break
            else:
                print("Dat is geen geldig optie nummer.")
        elif val == "stop":
            exit()
        else:
            print("Woeps dat is geen nummer!")
...
print("""Welk bestandstype wil je converteren?
+----------+----------+-----------+
| 1 Video  |  2 Audio |  3 Foto's |
+----------+----------+-----------+""")
# Welke converter moet worden gebruikt.
while True:
    val = input("Typ: stop om af te sluiten | Number:").strip()
    if val.isnumeric():
...
+-------+--------+--------+--------+""")
    while True:  # Zet nummer om in extensie voor de input bestanden.
        val = input("Typ: stop om af te sluiten | Welke extensie heeft het input bestand? Nummer:").strip()
        if val.isnumeric():
            gekozen_input_formaat_nummer = int(val)
            if 10 > gekozen_input_formaat_nummer > 0:
                break
            else:
                print("Dat is geen geldig extensie nummer. Probeer opnieuw.")
        elif val == "stop":
            exit()
        else:
            print("Woeps dat is geen nummer!")
    ...
    while True:
        val = input("Typ: stop om af te sluiten | To which extension do you want to convert the file(s)? Number?:").strip()
        if val.isnumeric():
...
""")
    while True:  # Zet nummer om in extensie voor de input bestanden.
        val = input("Typ: stop om af te sluiten | Welke extensie heeft het originele bestand? Nummer:")
        if val.isnumeric():
            gekozen_input_formaat_nummer = int(val)
            if 10 > gekozen_input_formaat_nummer > 0:
                break
    ...
    while True:
        val = input("Typ: stop om af te sluiten | Welke extensie moet het output bestand hebben? Nummer:")
        if val.isnumeric():
            gekozen_output_formaat_nummer = int(val)
            if 10 > gekozen_output_formaat_nummer > 0:
                break
